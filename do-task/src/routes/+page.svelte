<script lang="ts">
	...
	onMount(async () => {
		try {
			// Import auth service to check authentication

			
			// First check authentication
			await authService.checkAuth();
			
			// After auth check, if user is authenticated, load data
			if ($user) {
	...
	let completedTasks = $derived(
		($tasks || []).filter((task: Task) => task.status === TASK_STATUS.COMPLETED).length
	);
	...
	let inProgressTasks = $derived(
		($tasks || []).filter((task: Task) => task.status === TASK_STATUS.IN_PROGRESS).length
	);
	...
	// Using primitive $state values instead of an object with properties
	let taskTitle = $state('');
	let taskCategoryId = $state('');
	let taskStatus = $state(TASK_STATUS.TODO);
	let taskPriority = $state(PRIORITY.MEDIUM);

	async function handleQuickAddTask(event: Event) {
		event.preventDefault();
		if (!taskTitle || !taskCategoryId) return;
...
<div class="container mx-auto">
	...
	{:else if $error}
		<div class="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-8" role="alert">
			<p class="font-bold">Error working</p>
			<p>{$error}</p>
		</div>
	{:else}
		...
		<div class="bg-white dark:bg-gray-800 rounded-lg shadow p-6 mb-8">
			...
			{#if $categories.length === 0}
				<p class="text-muted text-center py-4">
					You need to create a category first before adding tasks.
				</p>
			{:else}
				...
				{:else}
					<ul class="space-y-3">
						{#each urgentTasks as task}
							<li
								class="border-l-4 border-red-500 bg-red-50 dark:bg-red-900/20 p-3 rounded-r-md hover:bg-red-100 dark:hover:bg-red-900/30 transition-colors"
							>
